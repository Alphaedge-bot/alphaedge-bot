...
V13.0.5 – UPDATED with ICT/SMC Zone-Based Risk Management
"""
...
class RiskGuardian:
    ...
    def __init__(self, event_bus: EventBus, state_manager: StateManager):
        ...
        # ============================================
        # NEW: Zone Detector for Dynamic SL/TP
        # ============================================
    ...
    async def handle_position_update(self, event: Event):
        ...
        # Calculate zone-based SL/TP for this position
        if token:
    ...
    # ============================================
    # NEW: Zone-Based Risk Management
    # ============================================
    ...
    async def calculate_position_zone_risk(self, token: str, position: Dict):
        ...
        try:
            ...
            # Get price data for zone detection
            ...
            # 1. Stop Loss: Find nearest demand zone below price
            nearest_demand = self.zone_detector.get_nearest_zone(current_price, -1)
            if nearest_demand:
                sl_price = nearest_demand.bottom * 0.998  # 0.2% below zone
            ...
            # 2. Take Profit: Find nearest supply zone above price
            ...
            # 3. Fallback: Fixed percentages if no zones found
            ...
            # Update position with zone-based SL/TP
            position['zone_sl'] = result['sl']
            position['zone_tp'] = result['tp']
            position['zone_sl_reason'] = result['sl_reason']
            position['zone_tp_reason'] = result['tp_reason']
            self.positions[token] = position
            await self.state_manager.set(f'position_{token}', position)
            ...
            logger.error(f"Zone risk calculation error for {token}: {e}")
    
    def _get_stop_loss_pct(self, cap_tier: str) -> float:
        """Get stop loss percentage by cap tier"""
        sl_map = {
            'micro_cap': 0.04,
            'small_cap': 0.05,
            'mid_cap': 0.07,
    ...
    async def handle_price_update(self, event: Event):
        ...
        if token and token in self.zone_risk_data:
            ...
            tp = zone_data.get('tp')
            if tp and price >= tp:
                logger.info(f"Zone TP reached for {token}: {price} >= {tp}")
                await self.event_bus.publish(Event(
                    ...
                    data={
                        ...
                        'price': price,
                        'tp': tp,
                        'reason': zone_data.get('tp_reason', 'zone_tp')
                    },
                    source=self.agent_id
                ))
            ...
            sl = zone_data.get('sl')
            if sl and price <= sl:
                logger.info(f"Zone SL reached for {token}: {price} <= {sl}")
                await self.event_bus.publish(Event(
                    ...
                    data={
                        ...
                        'price': price,
                        'sl': sl,
                        'reason': zone_data.get('sl_reason', 'zone_sl')
                    },
                    source=self.agent_id
                ))
