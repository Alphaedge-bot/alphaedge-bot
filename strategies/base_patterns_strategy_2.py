"""
AlphaEdge Strategy – Base Patterns Strategy 2
...
class BasePatternsStrategy2:
    """
    Base Patterns Strategy 2
    ...
    def __init__(self, event_bus, state_manager):
        ...
        self.strategy_id = "base_patterns_2"
        self.name = "Base Patterns Strategy 2"
        self.active = True
    ...
    async def handle_market_data(self, event: Event):
        ...
        if pattern == 'double_top' and level > 0:
            if price < level * (1 - self.pattern_threshold):
                if volume_ratio > self.volume_confirm:
                    signal = {
                        'strategy': self.strategy_id,
                        'type': 'sell',
                        'confidence': self.calculate_confidence(price, level, volume_ratio),
                        'pattern': 'double_top',
        ...
        elif pattern == 'double_bottom' and level > 0:
            if price > level * (1 + self.pattern_threshold):
                if volume_ratio > self.volume_confirm:
                    signal = {
                        'strategy': self.strategy_id,
